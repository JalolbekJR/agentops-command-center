...
export function BackToTop({ targetId }: { targetId: string }) {
  ...
  return (
    <button
      ...
      onClick={scrollToTop}
      className="back-to-top-control focus-ring fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-4 z-40 inline-flex h-9 items-center gap-1.5 rounded-md border border-white/[0.08] bg-slate-950/88 px-2.5 text-xs font-semibold text-slate-200 shadow-command backdrop-blur transition hover:bg-white/[0.08] sm:bottom-5 sm:right-5 sm:h-10 sm:gap-2 sm:px-3 sm:text-sm"
    >
