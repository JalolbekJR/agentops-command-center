...
import { auditListQuerySchema, parseProjectParams, parseQuery } from "@/server/api/validation";
...
export async function GET(request: Request, context: ProjectRouteContext) {
  return handleApiRequest(request, async ({ requestId }) => {
    ...
    const result = await withReadModels(requestId, "audit.read", (readModels) => readModels.listAuditEvents(projectId, query));
